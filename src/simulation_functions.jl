...
"""
function train(system,updates,trajTime=false)
    ...
    solOnline = solve_prob(probSens,cbs,tstopTimes) # solve ODE
...
function postProcess(system,trainStore,t_save,records,trajTime=false)
    ...
    map(t_save) do t 
        map(records) do el # for each post-process 
            el(prob,t,trainStore,postStore,getInputWeights(system))
...
function buildODETestProbF(sys,trajTime=false)
    ...
    function probF(W)
        if trajTime==false
            return ODEProblem(odeSyst, u0, (0.,sys.trajTime),vcat(pI,W))
        else 
            return ODEProblem(odeSyst, u0, (0.,trajTime),vcat(pI,W))
        end
    end
    return probF
end
...
"""
function simulate(system,updates,t_save,records,trajTime=false,trajTimePP=false,savePath="../Figures/deltaTe/")
    ...
    if trajTime==false # use default trajectory time
        trainStore = train(system,updates,system.trajTime) # train
    else 
        trainStore = train(system,updates,trajTime) # train
    end
