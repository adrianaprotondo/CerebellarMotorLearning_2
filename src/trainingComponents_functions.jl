...
function makeFOnlineUpdate(c::Union{LMSTrain,LMSCompute}) 
    function updateF(i,store)
        ...
        lms = ((store[:trajETr][:,i]-store[:trajETe][:,i])/(c.deltaTe-c.deltaTr) + etaScalar*noiseScalar ).*(store[:hTh][:,i].+ c.eta*noise) # calc lms from the store vals (normalize by window)
...
function makeFOnlineUpdate(c::GradientTrain,system)
    ...
    function updateF(i,store)
        ...
        if store[:keepTraining] # if keep training 
            taskError, grad = getGradLoss(pOutput,pI,prob,c.gradIndex) # get task grad
...
function makeCallback_update(times,updateFs,recordFs,namesR,store,N,W=false)
    ...
    function affect!(integrator)
        ...
        if W==false
            integrator.p[end-N+1:end] .+= dw # update weights
        else
            integrator.p[end-N+1:end] = W[:,i] # update weights
        end
...
function makeGradExtractor(syst::System,index)
    ...
    N = getN(syst)
    makeGradExtractor(numStates,numInputWeights,index,N)
end
...
"""
function solve_prob(probSens,cbs,tstopTimes)
    if isnothing(cbs)
        return solve(probSens, Tsit5()) 
    else
        return solve(probSens, Tsit5(), callback=cbs,tstops=tstopTimes)
