...
"""
function add_to_store!(c::Union{OnlineGradientTrain,OnlineGradientCompute},store,N)
...
"""
    For gaussian noise 
"""
function add_to_store!(c::Union{GaussianNoise,GaussianNoiseCompute},store,N)
    _add_to_store!(c,store,N)
...
"""
    For LMS training 
"""
function add_to_store!(c::Union{LMSTrain,LMSCompute},store,N)
    _add_to_store!(c,store,N) 
    get!(store,:trajETe,zeros(1,length(getTeTimes(c))))
    get!(store,:trajETr,zeros(1,length(getTrTimes(c))))
    get!(store,:hTh,zeros(N,length(getTrTimes(c))))
    # get!(store,:gradInstTr,zeros(N,length(getTrTimes(c))))
...
"""
function add_to_store!(c::GradientTrain,store,N)
...
function add_to_store!(c::LocalTaskDifficulty,store,N)
    _add_to_store!(c,store,N)
...
function add_to_store!(c::HessianAnalysis,store,N)
    _add_to_store!(c,store,N)
    get!(store,Symbol(:hessProj_,c.symbol),zeros(1,length(c.t))) # hessian projection
    get!(store,Symbol(:gradCorr_,c.symbol),zeros(1,length(c.t))) # correlation of gradient and vector
...
function add_to_store!(c::LearningPerformanceTrain,store,N)
    _add_to_store!(c,store,N)
    get!(store,:trainError,nothing) 
end
function add_to_store!(c::LearningPerformanceTask,store,N)
    _add_to_store!(c,store,N)
    get!(store,:taskError,zeros(1,length(c.t)))
end
function add_to_store!(c::GradCorr,store,N)
    _add_to_store!(c,store,N)
    get!(store,Symbol(:gradCorr_,c.symbol),zeros(1,length(c.t))) # correlation of gradient and vector
end
