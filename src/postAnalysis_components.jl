...
"""
function (o::PostSimulationAnalysis)(prob,t,trainStore,postStore,pI)
    # find each value we need to calculate the local task otherwise calculate
    reqs, rFs = requirements(o) # required functions 
    vals = map(rFs) do f
        f(prob,t,trainStore,postStore,pI) # run requirements
    end
...
function (o::LearningPerformanceTask)(prob,t,trainStore,postStore,pI)
    # find each value we need to calculate the local task otherwise calculate
    reqs, rFs = requirements(o)
    vals = map(rFs) do f
        f(prob,t,trainStore,postStore,pI)
    end
...
function (o::LearningPerformanceTrain)(prob,t,trainStore,postStore,pI)
    # at the last time step
    if t == o.t[end]
...
function (o::PlasticityVector)(prob,t,trainStore,postStore,pI)
...
function (o::GradNorm)(prob,t,trainStore,postStore,pI)
    i =  findfirst( x -> x == t, o.t) 
...
function (o::PvecNorm)(prob,t,trainStore,postStore,pI)
    i =  findfirst( x -> x == t, o.t) 
...
function (o::PostTrajectories)(prob,t,trainStore,postStore,pI)
...
"""
function (o::Grad)(prob,t,trainStore,postStore,pI)
    j = findfirst( x -> x == t, trainStore[:t_train])
    # println("Computing grad ", j)
    pOutput = get_store(trainStore,:weights,j)
    # println(pOutput)
    taskError, grad = getGradLoss(pOutput,pI,prob,o.gradIndex)
    i = findfirst( x -> x == t, o.t) 
    update_store!(postStore,pOutput,i,:weights) # update task error
...
function (o::TaskError)(prob,t,trainStore,postStore,pI)
    j = findfirst( x -> x == t, trainStore[:t_train])
    # println(j)
    pOutput = get_store(trainStore,:weights,j)
    taskError = getTaskError(pOutput,pI,prob,o.gradIndex)
    i = findfirst( x -> x == t, o.t) 
    update_store!(postStore,taskError,i,:taskError) # update task error
    return taskError
end
...
function (o::HessianProjection)(prob,t,trainStore,postStore,pI)
    j = findfirst( x -> x == t, trainStore[:t_train])
    ...
    if norm(val)==0
        ...
        proj = gradHessProj(pOutput,val,prob,pI,o.gradIndex)
    end 
    i = findfirst( x -> x == t, o.t)
    # println(size(proj))
...
function (o::HessianC)(prob,t,trainStore,postStore,pI)
    j = findfirst( x -> x == t, trainStore[:t_train])
    # println("computing hessian ", j)
    # println(j)
    pOutput = get_store(trainStore,:weights,j)
    val = get_store(trainStore,o.symbol,j) # plasticity vector
    taskError,grad,hessian = getGradAndHess(pOutput,pI,prob,o.gradIndex)
    i = findfirst( x -> x == t, o.t)
