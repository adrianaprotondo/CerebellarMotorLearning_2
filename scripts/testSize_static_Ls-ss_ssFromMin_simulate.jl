...
trajTime = 500.; # time of the reference trajectory for training 
trajTimeSS = 5000.; # traj time for ss calculation
trajTimePT = 6000. # pretrain trajectory time

trajTimePP = 1000.; # traj time for task loss calculation
...
####
numSim = 7; # number of different simulations (different initialisations)
seed = 18; # random seed to start with 
...
systemsSim = map(simI) do r
    # refF = ouFunDef(θ,sigma,μ,trajTime+dtI,dt,fc*dt) # use filtered O-U process as reference function 
...
mus = collect(0.001:0.02:0.2) # learning steps
...
#####
# Prepare paths and files to save variables wiht DrWatson
#####
...
if preTrain
    ...
    wssAll = map(1:length(simI)) do i # for each simulation pretrain with largest learning step and gradient descnt
        seed = simI[i]
        mkpath(plotsdir(path,"preTrain_$seed"))
        # for smallest net size 
        lossCutoff=0 # los cut off for gradient training 
        wss = preTrainF(systemsSim[i],musVar,Ns,trajTimePTs,dt,trainErrorIndex,trajTimePP,path, "preTrain_$seed/", "GradientTrain",lossCutoff) 
    end
    # save pretrain results to reuse if necessary 
    saveParamsPT = @strdict Ns trajTimePTs musVar simI wssAll
